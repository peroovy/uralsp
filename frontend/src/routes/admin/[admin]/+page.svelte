<script lang="ts">
	...
	export let userInfo: UserData = data.userInfo,
		competitionsInfo: Competitions = data.competitionsInfo,
		access_token: string = data.access_token;
	export let permission: string = data.permission,
		API: string = data.API;
	export let real_id: number = data.real_id;
	...
	function onKeyDown(event: KeyboardEvent) {
		...
		queryParams();
			...
			// TODO: Add a search as you type feature
		}
	}
	...
	async function editComp(id: number) {
		...
		let compRespond = await fetch(`${API}/competitions/${id}`, {
		method: "GET",
		headers: {
		...
		let compData = await compRespond.json();
		if (compRespond.ok) {
			comp = compData;	
		} else {
			printMsg(handleErrorMsg(compData), "error", alertCont);
		}
		...
		reg_s_date[0] +
		", at: " +
		reg_s_date[1].split(":").slice(0, -1).join(":");
		selectedCompRegEnd =
		reg_e_date[0] +
		", at: " +
		reg_e_date[1].split(":").slice(0, -1).join(":");
		selectedCompStart =
		start_date[0] +
		", at: " +
		start_date[1].split(":").slice(0, -1).join(":");
		...
		if (
		!selectedCompMonitors.includes(real_id) &&
		...
		let requestRespond = await fetch(`${API}/competitions/${comp.id}/requests`, {
		method: "GET",
		headers: {
			"Content-Type": "application/json",
			Authorization: "Bearer " + access_token,
		...
		let requestData = await requestRespond.json();
		if (requestRespond.ok) {
			requests = requestData;
			appLength = data.length;
		} else {
			printMsg(handleErrorMsg(requestData), "error", alertCont);
		}
	...
	async function downloadasExcel() {
		...
		if (selectedAppArray.size === 0) {
			printMsg("Please select at least one user", "error", alertCont);
			return;
		}
	...
	onMount(() => {
		...
		adminName = userInfo.email || userInfo.phone || "";
		}
		
		if (userInfo.surname) {
		adminName = userInfo.name + " " + userInfo.surname;
		} else {
		adminName = userInfo.name || userInfo.email || userInfo.phone || "";
		}
	...
	async function deleteComp(id: number): Promise<void> {
		...
		let compRequest = await fetch(`${API}/competitions/${id}`, {
		method: "DELETE",
		headers: {
			"Content-Type": "application/json",
			Authorization: "Bearer " + access_token,
		...
		let compData = await compRequest.json();
		if (compRequest.ok) {
			printMsg("Competition deleted successfully", "success", alertCont);
	...
	async function acceptApplication(id: number) {
		let confirmAccept = confirm(
		"Are you sure you want to accept this application?"
		);
		if (!confirmAccept) return;
		let description = prompt("Enter description:", "Accepted");

		let evalRequest = await fetch(`${API}/requests/${id}/process`, {
		method: "PATCH",
		headers: {
			"Content-Type": "application/json",
			Authorization: "Bearer " + access_token,
		...
		let evalData = await evalRequest.json();
		if (evalRequest.ok) {
			printMsg("Application accepted successfully", "success", alertCont);
			window.location.reload(); // refetch data
		} else {
			printMsg(handleErrorMsg(evalData), "error", alertCont);
		}
	}
...
<section class="admin-container">
  ...
  <nav
    class="navbar navbar-expand-lg navbar-light mb-5 sticky-top shadow-sm"
    bind:this={navBar}
  >
        ...
        >
          ...
          {#if per === "super_admin"}
            <li
              class="nav-item"
              on:click={() =>
                (window.location.href = "./contests/create-competition")}
            >
              <span class="nav-link" style:color="rgba(0,0,0,.55)">
                Create Competitions
              </span>
            </li>
          {/if}
  ...
  <div class="sliderCont" bind:this={sliderCont}>
    ...
    <div class="slide">
      <div class="card menu" bind:this={filterCont}>
        ...
        <button class="btn btn-light rounded-0" on:click={filter}>
          Filter
        </button>
      </div>
      <div
        class="container col-12 p-0 d-flex align-items-center justify-content-center mt-5"
      >
        <div
          class="row p-0 m-0 col-md gap-3 justify-content-center align-items-stretch"
          bind:this={compResults}
        >
          <div
            class="card col-md-5 comps compt-holder p-0 col-sm-6 shadow mt-0"
            style="max-width:500px; min-width:min-content"
          >
            ...
            <div class="card-body p-0 pt-1 shadow">
              ...
              {:else}
                <div class="alert alert-warning" role="alert">
                  ...
                  No competitions found, If you think that this is an error, please
                  refresh the page.
                </div>
              {/if}
            </div>
            <div
              class="container-fluid p-0 pt-3 bg-light d-flex justify-content-center"
            >
              <div
                class="row col-md-6 paginationNav justify-content-center align-items-center"
              >
                <ul class="pagination m-0 p-3">
                  {#each Array(Math.ceil(resultsNumber / itemPerpage)) as _, i}
                    <li
                      class="page-item page-link"
                      on:click={() => pagination(i)}
                    >
                      {i + 1}
                    </li>
                  {/each}
            ...
            <div class="card p-0 col-md-5" style="max-width:500px">
              <div class="card-body p-0 pt-1 shadow">
                <div class="noComp">
                  ...
                  <p>
                    No Competition Selected, please click on <i
                      class="fa fa-edit"
                    />.
                  </p>
                </div>
            ...
            <div class="card col-md-5 p-0" style="min-width: fit-content">
              ...
              >
                  ...
                  >
                    ...
                    <tbody>
                      {#each req as application, i}
                        <tr
                          bind:this={applicationBinds[i]}
                          id={application.id + ""}
                          class="text-center"
                        >
                          <td
                            ><li
                              on:click={() => selectApp(i)}
                          ...
                          <td class="text-center"
                            >{application.participants.length}</td
                          >
                          <td>
                            <div class="btn-group">
                              <button
                ...
                <div class="manage">
                  ...
                  <i on:click={selectAll} bind:this={selectAllBtn}>
                    >> Select all
                  </i>
                </div>
                <div
                  class="container-fluid p-0 pt-3 d-flex justify-content-center"
                >
                  <div
                ...
                <button
                  ...
                  on:click={() =>
                    (window.location.href =
                      base + "contests/" + selectedCompID)}
                >
                  <i class="fa fa-edit" /> Edit</button
                >
                <button
                  class="btn btn-danger rounded-0"
                  on:click={() => deleteComp(selectedCompID)}
                >
