<script lang="ts">
    ...
    let email: string | undefined, name: string | undefined, region: string | undefined, eduType: string | undefined, institute: string | undefined, year: string | undefined;
    ...
    function queryParams() {
        ...
        if (check) {
            printMsg("Please fill at least one field", "error", alertCont);
            return;
        }
    ...
    async function merge() {
        ...
        if (ids.length < 2 || ids.length > 2) {
            printMsg("Please sellect two users.", "error", alertCont);
            return;
        }
        alertCont.innerHTML = "";
        ...
        if (check) {
            // Send merge request
            let merege_request = await fetch(`${API}/users/merge`, {
                method: "POST",
                headers: {
            ...
            let merege = await merege_request.json();
            if (merege_request.ok) {
                printMsg("Users merged successfully.", "success", alertCont);
    ...
    async function getUsersData() {
        for (let i = 0; i < Array.from(selectedIds).length; i++) {
            ...
            // Request user data
            let userdata_request = await fetch(`${API}/users/${userId}`, {
                method: "GET",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${token}`,
            ...
            let userData = await userdata_request.json();
