<script lang="ts">
    ...
    let contest: undefined | CompetitionWithFields = data.contest;
    ...
    async function addFormField() {
        if (questionId == "") {
            printMsg("Please enter question id.", "error", alertCont);
            return;
        }
        if (questionTitle == "") {
            printMsg("Please enter question title", "error", alertCont);
            return;
        }
        if (questionType == "") {
            printMsg("Please select question type.", "error", alertCont);
            return;
        }
        // Duplicate check
        if (dupCheck()) {
            printMsg("Duplicate question id found!", "error", alertCont);
            return;
        }
        ...
        if (selectedOldField && selectedOldField.id == newField.id) {
            ...
            if (JSON.stringify(selectedOldField) != JSON.stringify(newField)) {
                ...
                printMsg("No changes on the selected field.", "success", alertCont);
            }
        } else {
            // Adding new field successfully
            printMsg("New field has been added.", "success", alertCont);
            // Add the new field to the Server
            let field_request = await fetch(`${API}/fields`, {
                method: "POST",
                headers: {
            ...
            let field_response = await field_request.json();
    ...
    async function updateField() {
        ...
        let questionId_request = await fetch(`${API}/fields/${questionId}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ` + access_token,
        ...
        let questionId_response = await questionId_request.json();
        ...
        formFields.filter((item, index) => {
            if (item.id === selectedOldField.id) {
                ...
                printMsg("The field has been updated.", "success", alertCont);
    ...
    async function removeField(): Promise<void> {
        if (selectedOldField) {
            if (confirm(`Do you want to delete the selected field with the id: ${selectedOldField.id}?`)) {
                ...
                let delteFiled_request = await fetch(`${API}/fields/${selectedOldField.id}`, {
                    method: "DELETE",
                    body: JSON.stringify({
                        fields: fields,
                    }),
                ...
                let delteFiled_response = await delteFiled_request.json();
        ...
        formFields.forEach((field) => {
            ...
            if (type == "textarea") {
                ...
                newField.innerHTML = `<div class="d-flex row col-12 m-0 justify-content-between align-items-baseline" data-id="${field.id}">
										...
										<input type="${type}" class="form-control" placeholder="Enter ${field.name}" value="${field.default_values[0]}" ${field.is_required ? "required" : ""}">
									</div>`;
            }
            ...
            removeBtn.addEventListener("click", () => {
                if ((removeBtn as HTMLElement).dataset.click) {
                    ...
                    addToPreview();
                    // Success message
                    printMsg("Field removed successfully", "success", alertCont);
                }
            });
        });
    }
    ...
    function addMonitor(id: number | undefined, printMessage: boolean = true): void {
        if (permission != "super_admin") return;
        if (id === undefined) {
            printMsg("error", "Please enter a valid ID", alertCont);
            return;
        }
        // Check if the id is a number
        if (isNaN(id)) {
            printMsg("error", "Please add a valid field ID", alertCont);
            return;
        }
        // Check if the id is already in the array
        if (monitors.includes(id)) {
            printMsg("This ID is already in the monitor list", "error", alertCont);
            return;
        }
        // Add to array
        monitors.push(id);
        ...
        // Success mesage
        if (printMessage) {
            printMsg(`Monitor with id: ${id} added to monitor list.`, "success", alertCont);
        }
        monitorId = undefined;
    }
    function nextSlide(part_name: string): void {
        ...
        if (part_name === "basicInfo") {
            ...
            if (!valid) {
                printMsg("Please fill all the fields", "error", alertCont);
                return;
            }
        } else if (part_name === "formFields") {
            if (formFields.length === 0) {
                printMsg("Please add atleast one field", "error", alertCont);
                return;
            }
        }
    ...
    async function create(): Promise<void> {
        ...
        let comp_request = await fetch(`${API}/competitions`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${access_token}`,
        ...
        let comp_request_data = await comp_request.json();
    ...
    async function update(whatToupdate: { all: boolean; form: boolean; admins: boolean }): Promise<void> {
        ...
        if (whatToupdate.form) {
            ...
            let temp_request = await fetch(`${API}/competitions/${contest?.id}/request-template`, {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${access_token}`,
            ...
            let temp_request_data = await temp_request.json();
            ...
            // update the fields
            let form_request = await fetch(`${API}/competitions/${contest?.id}/form`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${access_token}`,
            ...
            let form_request_data = await form_request.json();

            if (form_request_data.ok) {
        ...
        if (whatToupdate.admins) {
            let admins_list_request = await fetch(`${API}/competitions/${contest?.id}/admins`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${access_token}`,
            ...
            let admins_list_request_data = await admins_list_request.json();

            if (admins_list_request_data.ok) {
        ...
        if (whatToupdate.all) {
            let comp_update_request = await fetch(`${API}/competitions/${contest?.id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: `Bearer ${access_token}`,
            ...
            let comp_update_request_data = await comp_update_request.json();

            if (comp_update_request_data.ok) {
                printMsg("Competition updated successfully", "success", alertCont);
                setTimeout(() => {
                    window.location.reload();
                }, 1000);
            } else {
    ...
    <div class="container-fluid p-0">
        <div class="slideCont" bind:this={sliderCont}>
            <div class="slide" style="position: relative;">
                <div class="row shadow-sm justify-content-betweens align-items-center p-0 m-0" style="position: relative; z-index:10; background-color: white;">
                    <div class="card mb-4 col-md p-0 basicInfo" style="border: 10px solid white">
                        ...
                        <div class="card-body">
                            <form>
                                ...
                                <div class="form-group">
                                    <input bind:value={contestantsPerTeam} type="number" min="1" class="form-control" id="contestantsPerTeam" placeholder="Enter a Number ... " />
                                </div>
                            </form>
                        </div>
                    </div>
                    <lottie-player class="col-md lottie lottieInfo" src={lottieInfoSrc} style="width: 400px" background="transparent m-4" speed="1" loop nocontrols />
            ...
            <div class="slide" style="position: relative;">
                <div class="row shadow p-0 m-0 justify-content-between" style="position: relative; z-index: 10">
                    <div class="card col-md border-0 rounded-0">
                        ...
                        <div class="card-body form-creator">
                            <div class="form-group" style="position:relative; margin-bottom: 20px">
                                ...
                                <table class="table table-striped collapse" bind:this={suggestions} style="position: absolute; background-color: white; font-size:13px">
                                    <tbody>
                    ...
                    <div class="card col-md form-preview rounded-0 border-bottom-0">
                        ...
                        {#if formFields.length == 0}
                            <lottie-player src={lottieEmptySrc} background="transparent" speed="1" style="height: 300px; margin-top: 30px" loop nocontrols />
                        {/if}
                        <div class="card-body" bind:this={formPreview} />
                    </div>
                    <div bind:this={controlsCont[1]} style="background-color: white">
                        {@html slideContorls}
                    </div>
                </div>
            </div>
                ...
                <div class="slide">
                    <div class="row col-12 justify-content-center">
                        <div class="card p-0 mb-3 col-md-5">
                            ...
                            <div bind:this={controlsCont[2]} style="background-color: white">
                                {@html slideContorls}
                            </div>
                        </div>
                    </div>
...
<div class="choose-update shadow" bind:this={chooseUpdate}>
    <div class="card">
        <div class="card-body">
            ...
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" role="switch" aria-checked="false" id="flexSwitchCheck1" bind:checked={updateEverthing} />
                <label class="form-check-label" for="flexSwitchCheck1"> Everything </label>
            </div>
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheck2" aria-checked="false" bind:checked={updateAdmins} />
                <label class="form-check-label" for="flexSwitchCheck2"> Admins list </label>
            </div>
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheck3" aria-checked="false" bind:checked={updateForm} />
