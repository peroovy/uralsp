<script lang="ts">
    ...
    async function acceptApplication(id: number) {
        ...
        let app_process_request = await fetch(`${API}/requests/${id}/process`, {
            method: "PATCH",
            headers: {
        ...
        let app_process_request_json = await app_process_request.json();
    ...
    async function declineApplication(id: number) {
        ...
        let procces_update_requests = await fetch(`${API}/requests/${id}/process`, {
            method: "PATCH",
            headers: {
                "Content-Type": "application/json",
                Authorization: "Bearer " + access_token,
    ...
    function saveApp(index: number, mode = "msg"): string {
        ...
        for (let i = 0; i < template.length; i++) {
            ...
            let isRequired = (comp as CompetitionWithFields).fields.find((field) => field.id == fieldId)!.is_required;
    ...
    async function deleteApplication(id: number) {
        ...
        let del_app_request = await fetch(`${API}/requests/${id}`, {
            method: "DELETE",
            headers: {
                "Content-Type": "application/json",
        ...
        let del_app_request_json = await del_app_request.json();

        if (del_app_request.ok) {
            printMsg("Application deleted", "success", alertCont);
            setTimeout(() => {
                window.location.href = `${base}/participant/requests`;
            }, 1000);
        } else {
