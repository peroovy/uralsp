<script lang="ts">
  ...
  import type { Requests, Competitions, UserData } from "$lib/types";
  ...
  function renewApplication(id: number) {
    let confirmation = confirm(
      "Are you sure you want to renew your application?"
    );
    if (!confirmation) return;
...
<section class="participant-container">
      ...
      <div class="collapse navbar-collapse flex-grow-0" id="participant-menu">
        ...
        >
          <li class="nav-item" on:click={() => toSec("upcomming")}>
            <span class="nav-link active" id="upcomming">
              Upcomming registeration
            </span>
          </li>
  ...
  <div class="parts-container p-0" bind:this={sectionHolders}>
    ...
    <div class="part_4 d-flex justify-content-center align-items-start">
      <div
        class="row part_4 justify-content-center align-items-center gap-3 p-0 m-0"
        id="requests"
      >
        ...
        {:else}
          {#each requests as request}
            ...
            <div
              class="card p-0 border-0 shadow-sm"
              style="width: 500px; flex-flow: column nowrap;"
            >
              ...
              <div class="card-body">
                ...
                <table class="table table-striped table-hover">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td
                        >{createdAt.toDateString()}, {createdAt.getHours() +
                          ":" +
                          createdAt.getMinutes() +
                          ":" +
                          createdAt.getSeconds()}</td
                      >
                    </tr>
                    <tr>
                ...
                <div class="btn gap-2">
                  <button
                    ...
                    on:click={() => editApplication(request.competition)}
                  >
                    <li class="fa fa-edit" />
                    Edit
                  </button>
                  {#if request.status != "canceled" && request.status != "cancelled"}
                    <button
                      ...
                      on:click={() => cancelApplication(request.id)}
                    >
                  ...
                  {:else if request.status == "canceled" || request.status == "cancelled"}
                    <button
                      ...
                      on:click={() => renewApplication(request.id)}
                    >
