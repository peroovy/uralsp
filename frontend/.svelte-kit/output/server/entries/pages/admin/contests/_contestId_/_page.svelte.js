...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
    ...
    return `${
        (($$result.head += `${
            (($$result.title = `<title>App Name| Create contests</title>`), "")
        }<script defer src="${"https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"}" data-svelte="svelte-1xsxech"><\/script>`),
        "")
    }
...
<section class="${"createContest svelte-5fpjhc"}"><img class="${"d1 svelte-5fpjhc"}"${add_attribute("src", dotsSrc, 0)} alt="${""}">
	...
	<div class="${"container-fluid p-0"}"><div class="${"slideCont svelte-5fpjhc"}"${add_attribute(
        "this",
        sliderCont,
        0
    )}><div class="${"slide svelte-5fpjhc"}" style="${"position: relative;"}"><div class="${"row shadow-sm justify-content-betweens align-items-center p-0 m-0"}" style="${"position: relative; z-index:10; background-color: white;"}"><div class="${"card mb-4 col-md p-0 basicInfo"}" style="${"border: 10px solid white"}"><h1 class="${"card-title"}" style="${"padding: 15px; margin-bottom: 10px"}"><i class="${"fa fa-info-circle me-2 svelte-5fpjhc"}"></i>
							Basic info
						</h1>
						<div class="${"card-body"}"><form><div class="${"form-group"}"><h6>Contest name</h6>
								...
								<h6>Start</h6>
								<div class="${"row p-0"}"><div class="${"col-md-7 form-group"}"><input type="${"date"}" class="${"form-control svelte-5fpjhc"}" id="${"startDate"}" placeholder="${"Start date"}"${add_attribute(
        "value",
        startOn,
        0
    )}></div>
									<div class="${"col-md-5 form-group"}"><input type="${"time"}" class="${"form-control svelte-5fpjhc"}" id="${"startTime"}" placeholder="${"Start time"}"${add_attribute(
        "value",
        startAt,
        0
    )}></div></div>
								<h6>Registration start</h6>
								<div class="${"row p-0"}"><div class="${"col-md-7 form-group"}"><input type="${"date"}" class="${"form-control svelte-5fpjhc"}" id="${"regStart"}" placeholder="${"Registration start date"}"${add_attribute(
        "value",
        regStartOn,
        0
    )}></div>
									<div class="${"col-md-5 form-group"}"><input type="${"time"}" class="${"form-control svelte-5fpjhc"}" id="${"regStartTime"}" placeholder="${"Registration start time"}"${add_attribute(
        "value",
        regStartAt,
        0
    )}></div></div>
								<h6>Registration end</h6>
								<div class="${"row p-0"}"><div class="${"col-md-7 form-group"}"><input type="${"date"}" class="${"form-control svelte-5fpjhc"}" id="${"regEnd"}" placeholder="${"Registration end date"}"${add_attribute(
        "value",
        regEndOn,
        0
    )}></div>
									<div class="${"col-md-5 form-group"}"><input type="${"time"}" class="${"form-control svelte-5fpjhc"}" id="${"regEndTime"}" placeholder="${"Registration end time"}"${add_attribute(
        "value",
        regEndAt,
        0
    )}></div></div>
								<h6>Number of contestants per team</h6>
								<div class="${"form-group"}"><input type="${"number"}" min="${"1"}" class="${"form-control"}" id="${"contestantsPerTeam"}" placeholder="${"Enter a Number ... "}"${add_attribute(
        "value",
        contestantsPerTeam,
        0
    )}></div></form></div></div>
					<lottie-player class="${"col-md lottie lottieInfo svelte-5fpjhc"}"${add_attribute(
        "src",
        lottieInfoSrc,
        0
    )} style="${"width: 400px"}" background="${"transparent m-4"}" speed="${"1"}" loop nocontrols></lottie-player>
					<div${add_attribute("this", controlsCont[0], 0)}><!-- HTML_TAG_START -->${slideContorls}<!-- HTML_TAG_END --></div></div></div>
			<div class="${"slide svelte-5fpjhc"}" style="${"position: relative;"}"><div class="${"row shadow p-0 m-0 justify-content-between"}" style="${"position: relative; z-index: 10"}"><div class="${"card col-md border-0 rounded-0 svelte-5fpjhc"}"><h1 class="${"m-0 card-title d-flex align-items-center p-3 border-0 rounded-0"}"><i class="${"fa-brands fa-wpforms me-2"}"${add_styles(
        { "font-size": `30px` }
    )}></i>
							Create Field
						</h1>
						<div class="${"card-body form-creator"}"><div class="${"form-group"}" style="${"position:relative; margin-bottom: 20px"}"><label for="${"question"}">Question Id </label>
								...
								<table class="${"table table-striped collapse"}" style="${"position: absolute; background-color: white; font-size:13px"}"${add_attribute("this", suggestions, 0)}><tbody>${
        filtered.length != 0
            ? `${each(filtered, (res, i) => {
            ...
            : ``
							...
							${``}
							${``}
							<div class="${"form-group field_options"}"><div class="${"form-check form-switch svelte-5fpjhc"}"><input class="${"form-check-input"}" type="${"checkbox"}" id="${"Visible"}"${add_attribute(
        "checked",
        is_visible,
        1
    )}>
							...
							<div class="${"btn-group gap-1 col-12 justify-content-center align-items-center"}"><button class="${
        "btn btn-primary btn-sm btn-block mt-3 " + escape(flag_update ? "disabled" : "", true) + " svelte-5fpjhc"
    }">Add field</button>
								${
                                    flag_update
                                        ? `<button class="${"btn btn-secondary btn-sm btn-block mt-3 svelte-5fpjhc"}">Update field</button>
									<button class="${"btn btn-danger btn-sm btn-block mt-3 svelte-5fpjhc"}">Remove field</button>`
                                        : ``
                                }</div></div></div>
					<div class="${"card col-md form-preview rounded-0 border-bottom-0 svelte-5fpjhc"}"><h6 class="${"card-title form-preview-title border-bottom"}" style="${"margin: 0px -12px; padding: 15px"}"><i class="${"fa-regular fa-eye me-2"}"></i>
						...
						${
                            formFields.length == 0
                                ? `<lottie-player${add_attribute(
                                      "src",
                                      lottieEmptySrc,
                                      0
                                  )} background="${"transparent"}" speed="${"1"}" style="${"height: 300px; margin-top: 30px"}" loop nocontrols></lottie-player>`
                                : ``
                        }
			...
			${
                permission === "super_admin"
                    ? `<div class="${"slide svelte-5fpjhc"}"><div class="${"row col-12 justify-content-center"}"><div class="${"card p-0 mb-3 col-md-5 svelte-5fpjhc"}"><h1 class="${"card-header"}" style="${"padding: 20px"}"><i class="${"fa-solid fa-gear me-1"}"></i>
								Set admin
							</h1>
							...
							<div style="${"background-color: white"}"${add_attribute("this", controlsCont[2], 0)}><!-- HTML_TAG_START -->${slideContorls}<!-- HTML_TAG_END --></div></div></div></div>`
                    : ``
            }</div></div>
	<div class="${"alertCont svelte-5fpjhc"}"${add_attribute("this", alertCont, 0)}></div></section>
<div class="${"loading svelte-5fpjhc"}"${add_attribute(
        "this",
        loading,
        0
    )}><div class="${"spinner-border svelte-5fpjhc"}" role="${"status"}"><span class="${"visually-hidden"}">Loading...</span></div></div>
<div class="${"choose-update shadow svelte-5fpjhc"}"${add_attribute(
        "this",
        chooseUpdate,
        0
    )}><div class="${"card svelte-5fpjhc"}"><div class="${"card-body"}"><h5 class="${"card-title"}">What do you want to update?</h5>
			<div class="${"form-check form-switch svelte-5fpjhc"}"><input class="${"form-check-input"}" type="${"checkbox"}" role="${"switch"}" id="${"flexSwitchCheck1"}"${add_attribute(
        "checked",
        updateEverthing,
        1
    )}>
				<label class="${"form-check-label"}" for="${"flexSwitchCheck1"}">Everything </label></div>
			<div class="${"form-check form-switch svelte-5fpjhc"}"><input class="${"form-check-input"}" type="${"checkbox"}" role="${"switch"}" id="${"flexSwitchCheck2"}"${add_attribute(
        "checked",
        updateAdmins,
        1
    )}>
				<label class="${"form-check-label"}" for="${"flexSwitchCheck2"}">Admins list </label></div>
			<div class="${"form-check form-switch svelte-5fpjhc"}"><input class="${"form-check-input"}" type="${"checkbox"}" role="${"switch"}" id="${"flexSwitchCheck3"}"${add_attribute(
        "checked",
        updateForm,
        1
    )}>
