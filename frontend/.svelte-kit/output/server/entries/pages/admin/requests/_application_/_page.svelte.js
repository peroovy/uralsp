...
<div class="${"application svelte-wnp09j"}"><img class="${"d1 svelte-wnp09j"}"${add_attribute("src", dotsSrc, 0)} alt="${""}">
	...
	<div class="${"container-fluid d-flex justify-content-center align-items-center"}" style="${"min-height: calc(100vh - 38px); width: 100vw;"}"><div class="${"row m-0 justify-content-center align-items-stretch gap-0"}"><div class="${"card col-md border-0 rounded-0 basic-app-info svelte-wnp09j"}" style="${"min-width: min-content;"}"><h3 class="${"card-title mb-2 mt-4 ms-1 me-5"}"><span class="${"fa fa-info-circle ms-0 me-2 svelte-wnp09j"}"></span>
        ...
        return `${
            comp.persons_amount > 1
                ? `<button class="${"btn btn-light border-0 rounded-0 btn-block"}" type="${"button"}" data-bs-toggle="${"collapse"}" data-bs-target="${
                      "#appLicationNum" + escape(i, true)
                  }" aria-expanded="${"false"}" aria-controls="${"appLicationNum" + escape(i, true)}">Application Number: ${escape(i + 1)}
							</button>`
                : ``
        }
							<div class="${"collapse multi-collapse " + escape(i == 0 ? "show" : "", true)}" id="${"appLicationNum" + escape(i, true)}"${add_attribute("this", requestTemplates[i], 0)}>${
            comp.persons_amount > 1
...
<div class="${"loading svelte-wnp09j"}"${add_attribute(
        "this",
        loading,
        0
    )}><div class="${"spinner-border svelte-wnp09j"}" role="${"status"}"><span class="${"visually-hidden"}">Loading...</span></div></div>
