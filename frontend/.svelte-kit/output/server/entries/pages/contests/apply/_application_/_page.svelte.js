...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.contest === void 0 && $$bindings.contest && contest !== void 0)
    $$bindings.contest(contest);
  if ($$props.oldRequest === void 0 && $$bindings.oldRequest && oldRequest !== void 0)
    $$bindings.oldRequest(oldRequest);
...
<section class="${"container-fluid p-0 contestForm justify-content-center align-items-start svelte-akqv6t"}"><img class="${"d1 svelte-akqv6t"}"${add_attribute("src", dotsSrc, 0)} alt="${""}">
  ...
  <div class="${"row col-12 m-0 p-0 justify-content-center"}"><div class="${"card col-md-5 p-0 svelte-akqv6t"}" style="${"max-width: 500px"}"><nav class="${"navbar card-header svelte-akqv6t"}"><div class="${"container-fluid justify-content-left"}"><div class="${"d-flex navbar-brand mb-0 justify-content-left align-items-center gap-3 p-3 svelte-akqv6t"}"><li class="${"fa fa-certificate svelte-akqv6t"}"></li>
      ...
      <div class="${"card-body"}">${contest.persons_amount > 1 ? `<div class="${"form-field mb-3"}"><label for="${"teamName"}">Team Name <span class="${"text-danger"}"${add_styles({ "font-size": `19px` })}>*</span></label>
    ...
    return `${contest.persons_amount > 1 ? `<button class="${"btn mb-0 btn-light border-0 rounded-0 btn-block"}" type="${"button"}" data-bs-toggle="${"collapse"}" data-bs-target="${"#appLicationNum" + escape(i, true)}" aria-expanded="${"false"}" aria-controls="${"appLicationNum" + escape(i, true)}">Application Number: ${escape(i + 1)}
              </button>` : ``}
            
            <div class="${"collapse mt-0 multi-collapse bg-light " + escape(i == 0 ? "show" : "", true)}" id="${"appLicationNum" + escape(i, true)}"${add_attribute("this", requestTemplates[i], 0)}>${contest.persons_amount > 1 ? `<div class="${"form-field mb-3"}"><label for="${"teamName"}">Applicant Id <span class="${"text-danger"}"${add_styles({ "font-size": `19px` })}>*</span></label>
        ...
        <div class="${"btn-group col-12 pt-3 "}">
          <div class="${"btn-group col-12 gap-1 d-flex justify-content-center align-items-center"}">${``}
            ${permissions === "teacher" ? `<button class="${"btn btn-block btn-outline-primary rounded-0 svelte-akqv6t"}"><i class="${"fas fa-plus me-1"}"></i>
                Add another Application
              </button>` : ``}
            ${oldRequest === void 0 || Object.keys(oldRequest).length <= 0 ? `<button class="${"btn btn-block btn-primary rounded-0"}" style="${"background-color: #3490dc; border-color: #3490dc"}"><li class="${"fa fa-paper-plane me-1 svelte-akqv6t"}"></li>
                Submit
