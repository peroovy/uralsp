...
let load = async function load2({ params, parent, fetch }) {
    if (params.application === void 0) throw Error("Application id is not defined");
    redirect(307, "/");
    ...
    const oldRequest = oldRequestsJson.find((request) => request.owner == contestId || request.participants.includes(userId));
    return {
        contest: contestJson,
        oldRequest,
        userId,
