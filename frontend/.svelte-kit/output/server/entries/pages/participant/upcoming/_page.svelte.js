...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  let data = $page.data, upComming_competitions = data.upComming_competitions, userInfo = data.userInfo;
  sessionDuration();
  ...
  $$result.css.add(css);
  $$unsubscribe_page();
  return `${$$result.head += `${$$result.title = `<title>Upcoming registrations</title>`, ""}<script defer src="${"https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"}" data-svelte="svelte-7bk648"><\/script>`, ""}

<div class="${"loading svelte-fyijzj"}"${add_attribute("this", loading, 0)}><lottie-player${add_attribute("src", lottieNotFoundSrc, 0)} background="${"transparent"}" speed="${"1"}" style="${"width: 300px; height: 300px;"}" loop autoplay></lottie-player></div>
...
<section class="${"participant-container svelte-fyijzj"}"><img class="${"d1 svelte-fyijzj"}"${add_attribute("src", dotsSrc, 0)} alt="${""}">
  ...
  <div class="${"part_4 d-flex justify-content-center align-items-start svelte-fyijzj"}"><div class="${"row justify-content-center align-items-center gap-3 p-0 m-0"}" style="${"flex-flow: column nowrap; gap: 30px; width: max-content"}" id="${"upcomming"}">${upComming_competitions.length == 0 ? `<div class="${"text-center p-3 notFound svelte-fyijzj"}" style="${"width: fit-content; background: white"}"><lottie-player${add_attribute("src", lottieNotFoundSrc, 0)} background="${"transparent"}" style="${"max-width: 500px"}" speed="${"1"}" nocontrols></lottie-player>
    ...
    let diff = Date.parse(competition.registration_start) - Date.parse(Date()), regDay = Date.parse(competition.registration_start), days = Math.floor(diff / (1e3 * 60 * 60 * 24)), hours = Math.floor(diff % (1e3 * 60 * 60 * 24) / (1e3 * 60 * 60)), minutes = Math.floor(diff % (1e3 * 60 * 60) / (1e3 * 60));
    return `
        ...
        <div class="${"col-md"}"><div class="${"card shadow-sm border-0"}"><div class="${"card-header bg-light svelte-fyijzj"}"><nav class="${"navbar navbar-expand-lg bg-light navbar-light mb-3 align-items-center svelte-fyijzj"}"><div class="${"navbar-brand svelte-fyijzj"}"><h4 class="${"m-0 svelte-fyijzj"}">${escape(competition.name)}</h4></div>
              </nav></div>
            <div class="${"card-body gap-2"}"><table class="${"table"}"><tbody><tr><th scope="${"row"}"><i class="${"fa-solid fa-calendar"}"></i></th>
              ...
              <div class="${"btn btn-group gap-2"}"><button class="${"btn btn-primary"}"><a class="${"link-light svelte-fyijzj"}"${add_attribute("href", competition.link, 0)} target="${"_blank"}"><span class="${"fa fa-eye svelte-fyijzj"}"></span>
