...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
    ...
    let data = $page.data,
        ongoing_competition = data.ongoing_competition,
    ...
    return `${
        (($$result.head += `${
            (($$result.title = `<title>Ongoing registrations</title>`), "")
        }<script defer src="${"https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"}" data-svelte="svelte-1bo6wxh"><\/script>`),
        "")
    }

<div class="${"loading svelte-tdg4kv"}"${add_attribute("this", loading, 0)}><lottie-player${add_attribute(
        "src",
        lottieNotFoundSrc,
        0
...
<section class="${"participant-container svelte-tdg4kv"}"><img class="${"d1 svelte-tdg4kv"}"${add_attribute("src", dotsSrc, 0)} alt="${""}">
  ...
  <div class="${"part_4 d-flex justify-content-center align-items-start svelte-tdg4kv"}"><div class="${"row justify-content-center gap-4 justify-content-center align-items-center gap-3 p-0 m-0"}" style="${"flex-flow: column nowrap; gap: 30px; width: max-content"}" id="${"ongoing"}">${
        ongoing_competition.length == 0
            ? `<div class="${"text-center p-3 notFound svelte-tdg4kv"}" style="${"width: fit-content ;background: white"}"><lottie-player${add_attribute(
                  "src",
                  lottieNotFoundSrc,
                  0
          ...
          <small style="${"margin-top:-10px; display:block; opacity: 0.7"}">Please, try later.
            ...
            : `${each(ongoing_competition, (comp) => {
                  let diff = Date.parse(comp.registration_end) - Date.parse(Date()),
                      regDay = new Date(Date.parse(comp.registration_start)).toDateString(),
                      days = Math.floor(diff / (1e3 * 60 * 60 * 24)),
                      hours = Math.floor((diff % (1e3 * 60 * 60 * 24)) / (1e3 * 60 * 60)),
                      minutes = Math.floor((diff % (1e3 * 60 * 60)) / (1e3 * 60)),
                      ApplyLink = "/contests/apply/" + comp.id;
                  return `
          ...
          <div class="${"col-md"}"><div class="${"card shadow-sm border-0"}"><div class="${"card-header bg-light svelte-tdg4kv"}"><nav class="${"navbar navbar-expand-lg bg-light navbar-light mb-3 align-items-center svelte-tdg4kv"}"><div class="${"navbar-brand svelte-tdg4kv"}"><h4 class="${"m-0 svelte-tdg4kv"}">${escape(
                      comp.name
                  )}</h4></div>
                </nav></div>
              <div class="${"card-body gap-2"}"><table class="${"table"}"><tbody><tr><th scope="${"row"}"><i class="${"fa-solid fa-calendar"}"></i></th>
                ...
                <div class="${"btn btn-group gap-2"}"><button class="${"btn btn-primary"}"><a class="${"link-light svelte-tdg4kv"}"${add_attribute(
                      "href",
                      ApplyLink,
                      0
                  )} target="${"_blank"}"><span class="${"fa fa-check-square-o svelte-tdg4kv"}"></span>
                      <span class="${"ptn-count"}">Apply </span>
                    </a></button>
                  <button class="${"btn btn-primary"}"><a class="${"link-light svelte-tdg4kv"}"${add_attribute(
                      "href",
                      comp.link,
                      0
                  )} target="${"_blank"}"><span class="${"fa fa-eye svelte-tdg4kv"}"></span>
