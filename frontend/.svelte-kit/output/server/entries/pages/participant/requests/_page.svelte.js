...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
    ...
    return `${
        (($$result.head += `${
            (($$result.title = `<title>User requests</title>`), "")
        }<script defer src="${"https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"}" data-svelte="svelte-1o6vthh"><\/script>`),
        "")
    }

<div class="${"loading svelte-10xkaob"}"${add_attribute("this", loading, 0)}><lottie-player${add_attribute(
        "src",
        lottieNotFoundSrc,
        0
...
<section class="${"participant-container svelte-10xkaob"}"><img class="${"d1 svelte-10xkaob"}"${add_attribute("src", dotsSrc, 0)} alt="${""}">
  ...
  <div class="${"part_4 d-flex justify-content-center align-items-start svelte-10xkaob"}"><div class="${"row part_4 justify-content-center align-items-center gap-3 p-0 m-0 svelte-10xkaob"}" id="${"requests"}">${
        requests.length == 0
            ? `<div class="${"text-center p-3 notFound svelte-10xkaob"}" style="${"width: fit-content ;background: white"}"><lottie-player${add_attribute(
                  "src",
                  lottieNotFoundSrc,
                  0
          ...
          <div class="${"card p-0 border-0 shadow-sm"}" style="${"width: 500px; flex-flow: column nowrap;"}"><h4 class="${"card-header p-4 m-0 svelte-10xkaob"}"><li class="${"fa fa-paper-plane me-1 svelte-10xkaob"}"></li>
            ...
            <div class="${"card-body"}"><p class="${"request-description svelte-10xkaob"}">${escape(request.description ? request.description : "No description")}</p>
              ...
              <div class="${"btn gap-2"}"><button class="${"btn btn-primary btn-sm"}"><li class="${"fa fa-edit svelte-10xkaob"}"></li>
                ...
                ${
                    request.status != "canceled" && request.status != "cancelled"
                        ? `<button class="${"btn btn-danger btn-sm"}"><li class="${"fa fa-trash svelte-10xkaob"}"></li>
                    Cancel
                  </button>`
                        : `${
                              request.status == "canceled" || request.status == "cancelled"
                                  ? `<button class="${"btn btn-success btn-sm"}"><li class="${"fa fa-trash-restore svelte-10xkaob"}"></li>
                    Renew
                  </button>`
                                  : ``
                          }`
