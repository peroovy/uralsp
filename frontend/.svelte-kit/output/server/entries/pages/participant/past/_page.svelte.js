import { c as create_ssr_component, b as subscribe, d as add_attribute, v as validate_component, i as each, f as add_styles, e as escape } from "../../../../chunks/index.js";
...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  let data = $page.data, started_competitions = data.started_competitions, userInfo = data.userInfo;
  sessionDuration();
  ...
  $$result.css.add(css);
  $$unsubscribe_page();
  return `${$$result.head += `${$$result.title = `<title>Past registrations</title>`, ""}<script defer src="${"https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"}" data-svelte="svelte-x3fctm"><\/script>`, ""}
...
<section class="${"participant-container svelte-tdg4kv"}"><img class="${"d1 svelte-tdg4kv"}"${add_attribute("src", dotsSrc, 0)} alt="${""}">
  ...
  <div class="${"part_4 d-flex justify-content-center align-items-start svelte-tdg4kv"}"><div class="${"row justify-content-center align-items-center gap-3 p-0 m-0 "}" id="${"past"}"><div class="${"col-md"}">${started_competitions.length == 0 ? `<div class="${"text-center p-3 notFound svelte-tdg4kv"}" style="${"width: fit-content ;background: white"}"><lottie-player${add_attribute("src", lottieNotFoundSrc, 0)} background="${"transparent"}" style="${"max-width: 500px"}" speed="${"1"}" nocontrols></lottie-player>
            <h2 class="${"svelte-tdg4kv"}">No past registerations!</h2>
            <small style="${"margin-top:-10px; display:block; opacity: 0.7"}">Please, try later.
            </small></div>` : `${each(started_competitions, (comp) => {
    let StartDay = new Date(Date.parse(comp.started_at)).toDateString();
    return `

            <div class="${"col-md-5"}"${add_styles({ "width": `fit-content` })}><div class="${"card shadow-sm border-0"}"><div class="${"card-header bg-light svelte-tdg4kv"}"><nav class="${"navbar navbar-expand-lg bg-light navbar-light mb-3 align-items-center svelte-tdg4kv"}"><div class="${"navbar-brand svelte-tdg4kv"}"><h4 class="${"m-0 svelte-tdg4kv"}">${escape(comp.name)}</h4></div>
                  </nav></div>
                <div class="${"card-body gap-2"}"><table class="${"table"}"><tbody><tr><th scope="${"row"}"><i class="${"fa-solid fa-calendar"}"></i></th>
                        <td>Start date</td>
                        <td colspan="${"2"}">${escape(StartDay)}</td></tr>
                  ...
                  <div class="${"btn btn-group gap-2"}"><button class="${"btn btn-primary"}"><a class="${"link-light svelte-tdg4kv"}"${add_attribute("href", comp.link, 0)} target="${"_blank"}"><span class="${"fa fa-eye svelte-tdg4kv"}"></span>
