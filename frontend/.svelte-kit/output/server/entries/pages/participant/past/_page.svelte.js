...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
    ...
    let data = $page.data,
        started_competitions = data.started_competitions,
    ...
    return `${
        (($$result.head += `${
            (($$result.title = `<title>Past registrations</title>`), "")
        }<script defer src="${"https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"}" data-svelte="svelte-x3fctm"><\/script>`),
        "")
    }
...
<section class="${"participant-container svelte-tdg4kv"}"><img class="${"d1 svelte-tdg4kv"}"${add_attribute("src", dotsSrc, 0)} alt="${""}">
  ...
  <div class="${"part_4 d-flex justify-content-center align-items-start svelte-tdg4kv"}"><div class="${"row justify-content-center align-items-center gap-3 p-0 m-0 "}" id="${"past"}"><div class="${"col-md"}">${
        started_competitions.length == 0
            ...
            : `${each(started_competitions, (comp) => {
                  let StartDay = new Date(Date.parse(comp.started_at)).toDateString();
                  return `

            <div class="${"col-md-5"}"${add_styles({
                      width: `fit-content`,
                  })}><div class="${"card shadow-sm border-0"}"><div class="${"card-header bg-light svelte-tdg4kv"}"><nav class="${"navbar navbar-expand-lg bg-light navbar-light mb-3 align-items-center svelte-tdg4kv"}"><div class="${"navbar-brand svelte-tdg4kv"}"><h4 class="${"m-0 svelte-tdg4kv"}">${escape(
                      comp.name
                  )}</h4></div>
                  </nav></div>
