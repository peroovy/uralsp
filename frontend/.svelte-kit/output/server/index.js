...
function get_cookies(request, url) {
    ...
    const cookies = {
        get(name, opts) {
            ...
            const req_cookies = parse(request.headers.get("cookie") ?? "", {
                decode,
            });
...
const quoted = /* @__PURE__ */ new Set(["self", "unsafe-eval", "unsafe-hashes", "unsafe-inline", "none", "strict-dynamic", "report-sample", "wasm-unsafe-eval"]);
...
async function render_response({ branch, fetched, cookies, options, state, page_config, status, error: error2 = null, event, resolve_opts, action_result }) {
    var _a;
    if (state.prerendering) {
...
async function render_page(event, route, page, options, state, resolve_opts) {
    ...
    try {
        const nodes = await Promise.all([...page.layouts.map((n) => (n == void 0 ? n : options.manifest._.nodes[n]())), options.manifest._.nodes[page.leaf]()]);
