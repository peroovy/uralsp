...
from api.internal.auth.domain.services import SocialService, social_service
...
def test_updating_vkontakte(user: User) -> None:
    assert_update_social(user, social_service.update_vkontakte, "vkontakte_id")
...
def test_creating_user_from_vkontakte(vk_api: API) -> None:
    ...
    actual = social_service.try_get_or_create_user_from_vkontakte("")
...
def test_getting_vkontakte_id__unknown_user(vk_api: API, user: User) -> None:
    ...
    actual = social_service.try_get_vkontakte_id("")
...
@pytest.mark.unit
@pytest.mark.django_db
def test_updating_google(user: User) -> None:
    assert_update_social(user, social_service.update_google, "google_id")
...
def test_creating_user_from_google(google_api: id_token) -> None:
    ...
    actual = social_service.try_get_or_create_user_from_google("", "")
...
def test_getting_google_id__unknown_user(google_api: id_token, user: User) -> None:
    ...
    actual = social_service.try_get_google_id("", "")
...
def assert_update_social(user: User, update: Callable[[int, Optional[int]], int], attr: str) -> None:
    assert update(-1, 228) == 0
    assert getattr(User.objects.get(pk=user.pk), attr) != "228"

    assert update(user.id, 228) == 1
    assert getattr(User.objects.get(pk=user.pk), attr) == "228"

    assert update(user.id, None) == 1
    assert getattr(User.objects.get(pk=user.pk), attr) is None
