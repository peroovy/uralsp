...
def get_user_request_router(handlers: CurrentUserRequestsHandlers) -> Router:
    router = Router(tags=[USER_REQUESTS_TAG])

    router.add_api_operation(
        path="",
        methods=["GET"],
        view_func=handlers.get_request_from_user,
        response={200: CurrentUserRequestDetailsOut, 401: ErrorResponse, 404: ErrorResponse},
    )
