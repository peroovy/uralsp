...
class RequestHandlers:
    ...
    def create_request(self, request: HttpRequest, data: RequestIn = Body(...)) -> SuccessResponse:
        ...
        logger.info(log(CREATING_REQUEST, operation_id, STARTING, **log_kwargs))
        ...
        if not self._request_service.validate_users(competition, data):
            logger.success(log(CREATING_REQUEST, operation_id, OPERATION_IS_OVER__INVALID_USERS))
        ...
        if not self._request_service.validate_forms(data):
            logger.success(log(CREATING_REQUEST, operation_id, OPERATION_IS_OVER__INVALID_FORMS))
            raise UnprocessableEntityException(self.INVALID_FORMS, error=self.BAD_FORMS)

        with catch(CREATING_REQUEST, operation_id):
            self._request_service.create(request.user, data)

        logger.success(log(CREATING_REQUEST, operation_id, OPERATION_IS_OVER))
    ...
    def update_request(self, request: HttpRequest, request_id: int, data: FormsIn = Body(...)) -> SuccessResponse:
        ...
        logger.info(log(UPDATING_REQUEST, operation_id, STARTING, **log_kwargs))

        if not self._request_service.has_access(updater, user_request):
            logger.info(log(UPDATING_REQUEST, operation_id, OPERATION_IS_OVER__PERMISSION_DENIED))
            raise ForbiddenException()
        ...
        if not self._request_service.validate_users(user_request.competition, data):
            logger.success(log(UPDATING_REQUEST, operation_id, OPERATION_IS_OVER__INVALID_USERS))
            raise UnprocessableEntityException(self.INVALID_TEAM, error=self.BAD_USERS)
        ...
        if not self._request_service.validate_forms(request_in):
            logger.success(log(UPDATING_REQUEST, operation_id, OPERATION_IS_OVER__INVALID_FORMS))
            raise UnprocessableEntityException(self.INVALID_FORMS, error=self.BAD_FORMS)

        with catch(UPDATING_REQUEST, operation_id):
            self._request_service.update(user_request, request_in)

        logger.success(log(UPDATING_REQUEST, operation_id, OPERATION_IS_OVER))
        return SuccessResponse()
    ...
    def cancel_request(self, request: HttpRequest, request_id: int) -> SuccessResponse:
        ...
        logger.info(log(CANCELING_REQUEST, operation_id, STARTING, **log_kwargs))

        if not self._request_service.has_access(updater, user_request):
            logger.success(log(CANCELING_REQUEST, operation_id, OPERATION_IS_OVER__PERMISSION_DENIED))
            raise ForbiddenException()
        ...
        with catch(CANCELING_REQUEST, operation_id):
            self._request_service.cancel(user_request)

        logger.success(log(CANCELING_REQUEST, operation_id, OPERATION_IS_OVER))
        return SuccessResponse()
    ...
    def process_request(self, request: HttpRequest, request_id: int, data: ProcessIn) -> SuccessResponse:
        ...
        logger.info(log(PROCESSING_REQUEST, operation_id, STARTING, **log_kwargs))

        if not self._request_service.has_access(updater, user_request, only_admin=True):
            logger.success(log(PROCESSING_REQUEST, operation_id, OPERATION_IS_OVER__PERMISSION_DENIED))
            raise ForbiddenException()

        with catch(PROCESSING_REQUEST, operation_id):
            self._request_service.process(user_request, data)

        logger.success(log(PROCESSING_REQUEST, operation_id, OPERATION_IS_OVER))
        return SuccessResponse()
