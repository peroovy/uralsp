...
class CompetitionHandlers:
    ...
    def create_competition(self, request: HttpRequest, data: CompetitionIn = Body(...)) -> SuccessResponse:
        operation_id, creator = uuid.uuid4(), request.user
        ...
        with catch(CREATING_COMPETITION, operation_id):
    ...
    def update_competition(
        ...
        if not self._competition_service.has_access(competition_id, updater):
            logger.success(log(UPDATING_COMPETITION, operation_id, OPERATION_IS_OVER__PERMISSION_DENIED))
            raise ForbiddenException()

        self._assert_competition_in(data, UPDATING_COMPETITION, operation_id)

        with catch(UPDATING_COMPETITION, operation_id):
    ...
    def update_form(self, request: HttpRequest, competition_id: int, data: FormIn = Body(...)) -> SuccessResponse:
        ...
        if not self._competition_service.has_access(competition_id, updater):
            logger.success(log(UPDATING_FORM, operation_id, OPERATION_IS_OVER__PERMISSION_DENIED))
            raise ForbiddenException()

        if not self._competition_service.validate_fields(data.fields):
            logger.success(log(UPDATING_FORM, operation_id, OPERATION_IS_OVER__INVALID_FIELDS))
            raise UnprocessableEntityException(self.VALIDATION_FIELDS_ERROR, error=self.BAD_FIELDS)

        with catch(UPDATING_FORM, operation_id):
    ...
    def update_admins(self, request: HttpRequest, competition_id: int, data: AdminsIn = Body(...)) -> SuccessResponse:
        ...
        if not self._competition_service.validate_admins(data.admins):
            logger.success(log(UPDATING_ADMINS, operation_id, OPERATION_IS_OVER__INVALID_ADMINS))
            raise UnprocessableEntityException(self.VALIDATION_ADMINS_ERROR, error=self.BAD_ADMINS)

        with catch(UPDATING_ADMINS, operation_id):
    ...
    def _assert_competition_in(self, data: CompetitionIn, handler_name: str, operation_id: uuid.UUID) -> None:
        ...
        if not self._competition_service.validate_admins(data.admins):
            logger.success(log(handler_name, operation_id, OPERATION_IS_OVER__INVALID_ADMINS))
            raise UnprocessableEntityException(self.VALIDATION_ADMINS_ERROR, error=self.BAD_ADMINS)

        if not self._competition_service.validate_fields(data.fields):
            logger.success(log(handler_name, operation_id, OPERATION_IS_OVER__INVALID_FIELDS))
            raise UnprocessableEntityException(self.VALIDATION_FIELDS_ERROR, error=self.BAD_FIELDS)
